...
{
    Spec::Buffer* obj = ValueBinding<Spec::Buffer>::check( L, 1 );
	...
	try
	{
		obj->setAt( p, luaL_checknumber(L, dim + 2 ) );
...
int LuaBuffer::saveToFile(lua_State *L)
{
	...
	if( Util::top( L ) > 2 )
		kind = Util::toStr( L, 3 );
	Root::Ref<MemSpectrum> spec = new MemSpectrum( *obj ); // TODO: effizienter
...
int LuaBuffer::getAtomType(lua_State *L)
{
	...
	lua_pushstring(L, obj->getScale( dim - 1 ).getColor().getAtomLabel() ); 
	return 1;
}
...
int LuaBuffer::getIsotope(lua_State *L)
{
	...
	int dim = checkDim( L, 2, obj->getDimCount() );
	lua_pushstring(L, obj->getScale( dim - 1 ).getColor().getIsoLabel() ); 
	return 1;
}
...
int LuaBuffer::getAtPpm(lua_State *L)
{
	...
	if( dim == 2 )
		lua_pushnumber(L, obj->getAtPpm( p[ DimX ], p[ DimY ] ) );
	else if( dim == 1 )
		lua_pushnumber(L, obj->getAtPpm( p[ DimX ] ) );
	else
...
int LuaBuffer::getCorrelation(lua_State *L)
{
	...
	if( obj->getDimCount() > 2 )
		luaL_error( L, "Only one or two dimensions supported" );
    Spec::Buffer* b = ValueBinding<Spec::Buffer>::check( L, 2 );
	if( b->getDimCount() != obj->getDimCount() )
		luaL_error( L, "Buffer must have same number of dimensions" );
	for( int i = 0; i < obj->getDimCount(); i++ )
	{
		const Scale& a = b->getScale( i );
		const Scale& b = obj->getScale( i );
    ...
    Spec::Buffer* buf = ValueBinding<Spec::Buffer>::create( L );
	Buffer::calculate( Buffer::Correlate, *buf, *obj, *b );
	return 1;
}
...
int LuaBuffer::getConvolution(lua_State *L)
{
    ...
    Spec::Buffer* buf = ValueBinding<Spec::Buffer>::create( L );
	Buffer::calculate( Buffer::Convolute, *buf, *obj, *b );
	return 1;
}
...
{
	Spec::Buffer* rhs = ValueBinding<Spec::Buffer>::check( L, 1 );
	Spec::Buffer* lhs = ValueBinding<Spec::Buffer>::create(L);
	*lhs = *rhs;
	return 1;
}

const luaL_reg _methods[] =
{
	{"clone",  clone },
...
void LuaBuffer::install(lua_State * L)
{
	ValueBinding<Spec::Buffer>::install( L, "Buffer", _methods  );
}
