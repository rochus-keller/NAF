...
StarLexer::Token StarLexer::nextToken()
{
	...
	case DLR:
		if( (d_colNr + 1) < d_line.size() && d_line[d_colNr+1].unicode() == LCB )
		{
			d_colNr++;
...
bool StarLexer::isValid(const QChar &ch) const
{
	...
	if( uc <= 127 )
		return ch.isSpace() || ch.isPunct() || ch.isPrint() || uc == BEL;
	else if( uc >= 128 && uc <= 0xD7FF )
		return true;
	else if( uc >= 0xE000 && uc <= 0xFFFD )
		return true;
	else if( uc >= 0x10000 && uc <= 0x10FFF )
		return true;
	return false;
}
...
const char *StarLexer::Token::typeName() const
{
	static const char* names[] = {
		"Tag", "Quoted", "NonQuoted", "Multiline", "Loop", "Global", "Save", "Stop", "Data", "ListStart", "ListEnd", "ElemSep",
		"TableStart", "TableEnd", "RefStart", "RefEnd", "KeyValueSep", "Comment", "EndOfStream", "InvalidChar", "SyntaxError"
	};
