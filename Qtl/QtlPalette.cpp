...
{
	QPalette* lhs = ValueInstaller2<QPalette>::check( L, 1 );
...
{
	QPalette* lhs = ValueInstaller2<QPalette>::check( L, 1 );
	QPalette* rhs = ValueInstaller2<QPalette>::check( L, 2 );
...
int Palette::resolve(lua_State * L) // ( const QPalette & other ) const : QPalette
{
	QPalette* lhs = ValueInstaller2<QPalette>::check( L, 1 );
	QPalette* rhs = ValueInstaller2<QPalette>::check( L, 2 );
	QPalette* res = ValueInstaller2<QPalette>::create( L );
	*res = lhs->resolve( *rhs );
	return 1;
}
...
int Palette::QVariant(lua_State * L)
{
	QPalette* lhs = ValueInstaller2<QPalette>::check( L, 1 );
	Variant::push( L, QVariant::fromValue( *lhs ) );
	return 1;
}
int Palette::notEquals(lua_State * L) // ( const QPalette & p ) const : bool
{
	QPalette* lhs = ValueInstaller2<QPalette>::check( L, 1 );
	QPalette* rhs = ValueInstaller2<QPalette>::check( L, 2 );
...
static const luaL_reg _Palette[] = 
{
	...
	{ "QVariant", Palette::QVariant },
	{ Variant::notEquals, Palette::notEquals },
	{ Util::s_init, Palette::init },
	{ 0, 0 }
};
static const luaL_reg _PaletteMeta[] =
{
	{ "__eq", Palette::__eq },
	{ 0, 0 }
};
void Palette::install(lua_State * L){
	ValueInstaller2<QPalette>::install( L, _Palette, "QPalette" );
	ValueInstaller2<QPalette>::addMethodsToMeta( L, _PaletteMeta );
}
