...
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
...
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
	if(QPointF* f = ValueInstaller2<QPointF>::cast( L, 2 ))
...
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
	QRegion* r = ValueInstaller2<QRegion>::check( L, 2 );
...
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
	if(QRegion* f = ValueInstaller2<QRegion>::cast( L, 2 ))
		Util::push( L, lhs->intersects( *f ) );
	else if (QRectF* r = ValueInstaller2<QRectF>::cast( L, 2 ))
		Util::push( L, lhs->intersects( r->toRect() ) );
	return 1;
}
int MyRegion::isEmpty(lua_State * L) { return ValueInstaller2<QRegion>::getBool( L, &QRegion::isEmpty ); }
int MyRegion::rects(lua_State * L) // () const : QVector<QRect> 
{
	...
	{
		Util::push( L, i + 1 );
		res = ValueInstaller2<QRectF>::create( L );
		*res = r[i];
		lua_rawset( L, t );
	}
	return 1;
...
// table {QRect rects, QRect rects ...}
{
	...
	{size=lua_objlen(L,2);
	 ...
	 while (lua_next(L, 2) != 0)
		 {
			QRectF* r = ValueInstaller2<QRectF>::check( L, -1 );
...
int MyRegion::notEquals(lua_State * L) // (const QRegion & ) const : bool
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
	QRegion* r = ValueInstaller2<QRegion>::check( L, 2 );
...
{
	QRegion* lhs = ValueInstaller2<QRegion>::check( L, 1 );
	QRegion* rhs = ValueInstaller2<QRegion>::check( L, 2 );
	QRegion* res = ValueInstaller2<QRegion>::create( L );
	*res = *lhs + *rhs;
	return 1;
}
...
static const luaL_reg _Region[] = 
{
	...
	{ Variant::notEquals, MyRegion::notEquals },
...
void MyRegion::install(lua_State * L){
	ValueInstaller2<QRegion>::install( L, _Region );
	ValueInstaller2<QRegion>::addMethodsToMeta( L, _RegionMeta );
}
