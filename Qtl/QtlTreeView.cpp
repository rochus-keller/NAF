...
{
	QTreeView* obj = ObjectHelper<QTreeView>::check( L, 1);
...
{
	QTreeView* obj = ObjectHelper<QTreeView>::check( L, 1);
	ObjectPeer::pushPeer( L, obj->header(), true);
	return 1;
}
int TreeView::indexAbove(lua_State * L) // ( const QModelIndex & index ) const : QModelIndex 
{
	QTreeView* obj = ObjectHelper<QTreeView>::check( L, 1);
	QModelIndex* index = ValueInstaller2<QModelIndex>::check( L, 2 );
	QModelIndex* res = ValueInstaller2<QModelIndex>::create( L );
...
{
	QTreeView* obj = ObjectHelper<QTreeView>::check( L, 1);
	QModelIndex* parent = ValueInstaller2<QModelIndex>::check( L, 3 );
...
void TreeView::install(lua_State * L){
	ObjectInstaller<QTreeView>::install( L, _TreeView, ObjectInstaller<QAbstractItemView>::className());
}
