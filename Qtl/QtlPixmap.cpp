...
{
	QPixmap* obj = ValueInstaller2<QPixmap>::check( L, 1 );
...
// int Pixmap::defaultDepth(lua_State * L) { return ValueInstaller2<QPixmap>::getInt( L, &QPixmap::defaultDepth ); }
// int Pixmap::defaultOptimization(lua_State * L) { return ValueInstaller2<QPixmap>::getEnum( L, &QPixmap::defaultOptimization ); } 
int Pixmap::depth(lua_State * L) { return ValueInstaller2<QPixmap>::getInt( L, &QPixmap::depth ); } 
...
int Pixmap::fill(lua_State * L) // ( const QColor & ) und ( const QWidget *, int, int ) 
{
	Util::expect( L, 2 );
	QPixmap* obj = ValueInstaller2<QPixmap>::check( L, 1 );
	if( QColor* clr = ValueInstaller2<QColor>::cast( L, 2 ) )
	{
		obj->fill( *clr );
	}else
	{
...
int Pixmap::hasAlpha(lua_State * L) { return ValueInstaller2<QPixmap>::getBool( L, &QPixmap::hasAlpha ); } 
int Pixmap::hasAlphaChannel(lua_State * L) { return ValueInstaller2<QPixmap>::getBool( L, &QPixmap::hasAlphaChannel ); }
int Pixmap::height(lua_State * L) { return ValueInstaller2<QPixmap>::getInt( L, &QPixmap::height ); } 
int Pixmap::isNull(lua_State * L) { return ValueInstaller2<QPixmap>::getBool( L, &QPixmap::isNull ); }
int Pixmap::isQBitmap(lua_State * L) { return ValueInstaller2<QPixmap>::getBool( L, &QPixmap::isQBitmap ); }
...
int Pixmap::save(lua_State * L) // ( const QString &, const char *, int ) or ( QIODevice *, const char *, int ) const : bool 
{
	...
	{
		obj->save( Util::toStr( L, 2 ), Util::toStr( L, 3 ), q );
	}
	return 0;
}
...
{
	QPixmap* obj = ValueInstaller2<QPixmap>::check( L, 1 );
	QPixmap* p = ValueInstaller2<QPixmap>::check( L, 2 );
...
int Pixmap::copy(lua_State * L) // ( const QRect & rect = QRect() ) const : QPixmap
{
	...
	QRect rect;
	if( Util::top(L) > 1 )
		rect = ValueInstaller2<QRectF>::check( L, 2 )->toRect();
	*res = obj->copy( rect );
	return 1;
}
int Pixmap::serialNumber(lua_State * L) { return ValueInstaller2<QPixmap>::getInt( L, &QPixmap::serialNumber ); } 
// int Pixmap::setDefaultOptimization(lua_State * L) { return ValueInstaller2<QPixmap>::setEnum( L, &QPixmap::setDefaultOptimization ); } 
int Pixmap::setMask(lua_State * L) // ( const QBitmap & ) 
{
	QPixmap* obj = ValueInstaller2<QPixmap>::check( L, 1 );
	QBitmap* b = ValueInstaller2<QBitmap>::check( L, 2 );
	obj->setMask( *b );
	return 0;
}
...
int Pixmap::width(lua_State * L) { return ValueInstaller2<QPixmap>::getInt( L, &QPixmap::width ); } 
...
void Pixmap::install(lua_State * L){
	ValueInstaller2<QPixmap>::install( L, _Pixmap );
}
