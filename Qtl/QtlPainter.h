...
namespace Qtl
{
	...
	public:		
		...
		static int device(lua_State * L); // const : QPaintDevice * 
...
// ( int x, int y, const QImage & image, int sx = 0, int sy = 0, int sw = -1, int sh = -1, Qt::ImageConversionFlags flags = Qt::AutoColor )
		static Qt::ImageConversionFlags getFlags(lua_State * L, int position);
		...
		static Qt::FillRule getfillRule(lua_State * L, int position);
