...
{
	QIcon* obj = ValueInstaller2<QIcon>::check( L, 1 );
...
int Icon::addFile(lua_State * L) // ( const QString &, const QSize &, Mode, State )  
{
	...
	obj->addFile( Util::toStr(L,2), s->toSize(), mode, state );
	return 0;
}
int Icon::addPixmap(lua_State * L) // ( const QPixmap &, Mode, State ) 
{
	...
	if( Util::top(L) > 3 )
		state = (QIcon::State)Util::toInt( L, 4 );
	QPixmap* s = ValueInstaller2<QPixmap>::check( L, 2 );
	obj->addPixmap( *s, mode, state );
	return 0;
}
int Icon::isNull(lua_State * L) { return ValueInstaller2<QIcon>::getBool( L, &QIcon::isNull ); } 
...
int Icon::pixmap(lua_State * L) // ( int, Mode, State ) const : QPixmap 
{
	...
	if( QSizeF* s = ValueInstaller2<QSizeF>::cast( L, 2 ) )
	{
		if( Util::top(L) > 2 )
			mode = (QIcon::Mode)Util::toInt( L, 3 );
		if( Util::top(L) > 3 )
...
int Icon::serialNumber(lua_State * L) { return ValueInstaller2<QIcon>::getInt( L, &QIcon::serialNumber ); } 
...
void Icon::install(lua_State * L){
	ValueInstaller2<QIcon>::install( L, _Icon );
}
