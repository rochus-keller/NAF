...
void PeakModelView::draw(Canvas & p, const Allocation & a)
{
	if( !d_show || d_space.isNull() )
		return;
	try
	{
		...
		Coord i;
		Coord pos;
		if( area->inUse( DimY ) )	// Kurve verläuft vertikal
		{
			Coord pixmax = a.getWidth() - 20;
			Amplitude step = pixmax / ( amax - amin );	// Float-Range pro Pixel
			Coord zero = ( 0 - amin ) * step;				// Pixel-Position der Nullinie
			...
			if( d_grenze )
			{
				...
				Coord j = area->toTwip( p0, a.getTop(), area->getDim() );
				pos = ( aval - amin ) * step;
				i = area->toTwip( p0 + 
		...
		}else if( area->inUse( DimX ) )	// Kurve verläuft horizontal
		{
			Coord pixmax = a.getHeight() - 20;
			Amplitude step = pixmax / ( amax - amin );	// Float-Range pro Pixel
			Coord zero = ( 0 - amin ) * step;				// Pixel-Position der Nullinie
			...
			if( d_grenze )
			{
				...
				Coord j = area->toTwip( p0, a.getLeft(), area->getDim() );
				pos = ( aval - amin ) * step;
				i = area->toTwip( p0 + 
						par.d_width[ DimX ] * 0.5 * par.d_tol[ DimX ], 
						a.getLeft(), area->getDim() );
...
void PeakModelView::handle( Root::Message & msg )
{
	...
	{
		// TODO: nur gewisse Elemente von PeakSpace aktualisieren
		if( d_show && !d_space.isNull() )
			damageAll();
	}
	END_HANDLER();
