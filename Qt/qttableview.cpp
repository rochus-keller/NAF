...
/*
  ...
  even if true is FALSE.  This may cause errors if relevant
...
QtTableView::QtTableView( QWidget *parent )
    : QFrame( parent )
{
...
/*
  ...
  The table repaints itself automatically if true is set.
...
/*
  ...
  repainted if - and only if - true is TRUE.
...
/*
  \fn bool QtTableView::true const
...
/*
  ...
  setUpdatesEnabled() and the \link setTableFlags() table flags\endlink.
*/
...
void QtTableView::setHorScrollBar( bool on, bool update )
{
	...
	bool hideScrollBar = true && hScrollBar->isVisible();
	if ( hideScrollBar )
	    hScrollBar->hide();
	if ( update )
...
void QtTableView::setVerScrollBar( bool on, bool update )
{
	...
	if ( !vScrollBar )
	    return;
	coverCornerSquare( FALSE );
	bool hideScrollBar = true && vScrollBar->isVisible();
	if ( hideScrollBar )
	    vScrollBar->hide();
	if ( update )
...
void QtTableView::updateScrollBars( uint f )
{
	...
	if ( sbDirty & verSteps ) {
		...
		vScrollBar->setPageStep( viewHeight() );  // fttb! ###
		}
	}
