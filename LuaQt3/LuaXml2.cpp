...
typedef RefBinding<DomDocument> DomDocumentValue;
typedef RefBinding<DomElement> DomElementValue;
typedef RefBinding<DomText> DomTextValue;
...
{
    DomDocument* obj = DomDocumentValue::check( L, 1 );
...
{
    DomElement* obj = DomElementValue::check( L, 1 );
...
int LuaDomElement2::createElement(lua_State *L)
{
    DomElement* obj = DomElementValue::check( L, 1 );
	// Param: Name
	// Erzeugt ein neues Element und fgt es ans Ende der Liste
...
{
    DomText* obj = DomTextValue::check( L, 1 );
	pushNode( L, obj->getPrev() );
	return 1;
}
