...
{
	QtObject<LuaCanvas2>::create( L, new LuaCanvas2(0) );
	return 1;
}
...
static int doscript(lua_State *L)
{
	...
	if( status == 0 )
	{
        status = lua_pcall( L, 0, LUA_MULTRET, 0 ); // hier ok, da doscript nur von Lua selber aufgerufen
	}
	if( status != 0 )
	{
...
static int require(lua_State *L)
{
	...
	if( s->isCompiled() )
		return 0;	// Nur ausfhren, wenn nicht bereits ausgefhrt.
	int status = luaL_loadbuffer( L, s->getCode(), strlen( s->getCode() ), name );
	if( status == 0 )
	{
        status = lua_pcall( L, 0, LUA_MULTRET, 0 ); // hier ok, da require nur von Lua selber aufgerufen wird
	}
	if( status != 0 )
	{
		// Auf Stack befindet sich die Fehlermeldung
	...
	s->setCompiled();
	// TODO: es soll package.loaded[modname] zurckgegeben werden
	return 0;
}
...
static int loadImage( lua_State* L )
{
    ...
    QImage* img = QtValue<QImage>::create( L );
...
{
	const char* path = luaL_checkstring( L, 1 );
    QPixmap* img = QtValue<QPixmap>::create( L );
...
static int createContour( lua_State* L )
{
	Buffer* buf = 0;
	if( lua_gettop(L) > 0 )
        buf = QtValue<Buffer>::check( L, 1 );
	if( buf && buf->getDimCount() != 2 )
		luaL_error( L, "Buffer must have two dimensions" );
    LuaContourPlot2* plot = QtValue<LuaContourPlot2>::create( L );
	if( buf )
...
static int createSlice( lua_State* L )
{
	...
	Buffer* buf = 0;
	if( lua_gettop(L) > 1 )
        buf = QtValue<Buffer>::check( L, 2 );
	if( buf && buf->getDimCount() != 1 )
		luaL_error( L, "Buffer must have one dimension" );
    LuaSlicePlot2* plot = QtValue<LuaSlicePlot2>::create( L );
	plot->init( dim );
	if( buf )
		plot->model()->setBuffer( *buf );
	return 1;
...
static int pushCursor( lua_State* L )
{
	...
	if( i <= Lexi::Viewport::CurNone || i >= Lexi::Viewport::CurForbidden )
		luaL_argerror( L, 1, "invalid cursor id" );
	Lexi::Viewport::pushCursor( (Lexi::Viewport::Cursor) i );
	return 0;
}
...
static int openAttributeEditor( lua_State* L )
{
	...
	if( lua_gettop(L) > 1 )
	{
		...
		od = rep->findObjectDef( ValueBindingBase::getTypeName( L, 1 ) );
