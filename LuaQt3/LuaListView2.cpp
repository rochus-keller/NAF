...
#include <Qtl2/QtlTreeWidgetItem.h>
using namespace Lua;
using namespace Qtl;
...
class LuaListItem2 : public TreeWidgetItem
{
public:
	...
	LuaListItem2( LuaListItem2* p ):TreeWidgetItem( p, QTreeWidgetItem::Type ) {}
	LuaListItem2( LuaListView2* p ):TreeWidgetItem( p, QTreeWidgetItem::Type ) {}
};


void LuaListView2::install( lua_State *L )
{
	QtObject<LuaListView2,QTreeWidget,NotCreatable>::install( L, "ListView", LuaListView2::methods );
	PtrBinding<LuaListItem2,TreeWidgetItem>::install( L, "ListViewItem", LuaListItem2::methods );
}
...
{
	LuaListView2* obj = QtObject<LuaListView2>::check( L, 1 );
	PtrBinding<LuaListItem2>::create( L, new LuaListItem2( obj ) );
	return 1;
}
...
//        obj->setColumnCount( obj->columnCount() - 1 );
    // delete obj->headerItem( luaL_checknumber( L, 2 ) - 1 );
...
{
	QTreeWidget* obj = QtObject<QTreeWidget>::check( L, 1 );
    QTreeWidgetItem* w = PtrBinding<TreeWidgetItem>::check( L, 2 );
	CHECK_ALIVE(w);
	obj->scrollToItem( w );
	return 0;
}
...
int LuaListView2::_getFirstChild(lua_State *L)
{
	...
	if( obj->topLevelItemCount() == 0 )
        ...
        TreeWidgetItem::create( L, obj->topLevelItem( 0 ) );
	return 1;
}
...
{
    // QTreeWidget* obj = QtObject<QTreeWidget>::check( L, 1 );
    //obj->setItemMargin( luaL_checknumber( L, 2 ) );
...
{
    QTreeWidgetItem* obj = PtrBinding<TreeWidgetItem>::check( L, 1 );
	CHECK_ALIVE(obj);
...
int LuaListItem2::setIcon(lua_State *L)
{
	...
	// Index 1 beginnend
	if( QPixmap* img = QtValue<QPixmap>::cast( L, 3 ) )
	{
		obj->setIcon( luaL_checknumber( L, 2 ) - 1, *img );
	}else if( QIcon* img = QtValue<QIcon>::cast( L, 3 ) )
	{
		obj->setIcon( luaL_checknumber( L, 2 ) - 1, *img );
...
int LuaListItem2::getFirstChild(lua_State *L)
{
	...
	if( obj->childCount() == 0 )
        ...
        TreeWidgetItem::create( L, obj->child(0) );
	return 1;
}
...
int LuaListItem2::getListView(lua_State *L)
{
    QTreeWidgetItem* obj = PtrBinding<TreeWidgetItem>::check( L, 1 );
	CHECK_ALIVE(obj);
    QtObject<QTreeWidget>::create( L, obj->treeWidget() );
	return 1;
}

int LuaListItem2::createItem(lua_State *L)
{
	LuaListItem2* obj = PtrBinding<LuaListItem2>::check( L, 1 );
	CHECK_ALIVE(obj);
	PtrBinding<LuaListItem2>::create( L, new LuaListItem2( obj ) );
	return 1;
}
...
{
    if( LuaWidgetCallbacks::pushCallbackAndMe( this, LuaWidget2::RightPressed ) )
	{
        if( item == 0 )
			lua_pushnil( LuaWidgetCallbacks::getLua() );
		else
            TreeWidgetItem::create( LuaWidgetCallbacks::getLua(), item );
		lua_pushnumber( LuaWidgetCallbacks::getLua(), p.x() );
		lua_pushnumber( LuaWidgetCallbacks::getLua(), p.y() );
		lua_pushnumber( LuaWidgetCallbacks::getLua(), c + 1 );
...
{
    if( LuaWidgetCallbacks::pushCallbackAndMe( this, LuaWidget2::Selection ) )
	{
...
{
    if( LuaWidgetCallbacks::pushCallbackAndMe( this, LuaWidget2::Expanded ) )
	{
        if( item == 0 )
			lua_pushnil( LuaWidgetCallbacks::getLua() );
		else
...
{
    if( LuaWidgetCallbacks::pushCallbackAndMe( this, LuaWidget2::Collapsed ) )
	{
        if( item == 0 )
			lua_pushnil( LuaWidgetCallbacks::getLua() );
		else
