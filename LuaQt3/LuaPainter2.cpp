...
{
	QPixmap* img = QtValue<QPixmap>::check( L, 1 );
	// Params: none
...
{
	QImage* img = QtValue<QImage>::check( L, 1 );
	// Params: none
	// Returns: number (w), number (h)
	lua_pushnumber(L, img->width() ); 
	lua_pushnumber(L, img->height() ); 
...
{
    ValueBinding<LuaPainter2>::install( L, "Painter", methods );
    QtValue<QPixmap>::addMethods( L, s_pixmap_methods );
    QtValue<QImage>::addMethods( L, s_image_methods );
}

int LuaPainter2::drawEllipse(lua_State *L)
{
    LuaPainter2* obj = ValueBinding<LuaPainter2>::check( L, 1 );
	CHECK_ACTIVE();
	obj->d_p.drawEllipse( 
