...
{
    QButtonGroup* obj = QtObject<QButtonGroup>::check( L, 1);
	if(Util::isNum( L, 3))
	{
...
{
    QButtonGroup* obj = QtObject<QButtonGroup>::check( L, 1);
    QtObject<QAbstractButton>::create( L, obj->button( Util::toInt( L, 2) ) );
	return 1;
}
int ButtonGroup::buttons(lua_State * L) // const : QList<QAbstractButton *> 
{
	...
	{
		Util::push( L, i + 1 );
        QtObject<QAbstractButton>::create( L, r[i] );
        lua_rawset( L, t );
	}
	return r.size();
}
...
{
    QButtonGroup* obj = QtObject<QButtonGroup>::check( L, 1);
    QtObject<QAbstractButton>::create( L, obj->checkedButton() );
	return 1;
}
...
void ButtonGroup::install(lua_State * L){
    QtObject<QButtonGroup,QObject>::install( L, "QButtonGroup", _ButtonGroup );
}
