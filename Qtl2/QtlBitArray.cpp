...
{
    QBitArray* obj = QtValue<QBitArray>::check( L, 1 );
    Util::push( L, obj->at( Util::toInt( L, 2 ) ) );
	return 1;
}
...
int BitArray::setBit(lua_State * L) // ( int ) 
{
	...
	if( lua_gettop(L) > 2 ) // ( int, bool ) 
        obj->setBit( Util::toInt( L, 2 ), Util::toBool( L, 3 ) );
	else
        obj->setBit( Util::toInt( L, 2 ) );
	return 0;
}
...
{
    QBitArray* obj = QtValue<QBitArray>::check( L, 1 );
    Util::push( L, obj->testBit( Util::toInt( L, 2 ) ) );
	return 1;
}
int BitArray::toggleBit(lua_State * L) // ( int ) : bool 
{
    QBitArray* obj = QtValue<QBitArray>::check( L, 1 );
    Util::push( L, obj->toggleBit( Util::toInt( L, 2 ) ) );
	return 1;
}
...
void BitArray::install(lua_State * L){
    QtValue<QBitArray>::install( L, "QBitArray", _BitArray );
}
