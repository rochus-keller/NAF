...
namespace Lexi
{
	...
	//. Mouse-Events empfngt und daraus ein Rubber-Band in Allocation
	...
	public:
		...
		//. Gebe das Rubber-Rectangle zurck.
		void draw( Canvas&, const Allocation&);

		SelectorCmd(Glyph* controller = nil, QColor = Qt::lightGray );
		// Wenn bergeordneter GlyphCommander nicht bereits den Fokus hat, muss
		// er hier in controller angegeben werden.
	protected:
		Result handleMousePress( Viewport&, const Allocation&, const MouseEvent& );
		virtual ~SelectorCmd();

		Result handleKeyPress( Viewport&, const Allocation&, const KeyEvent& );
		Result handleMouseRelease( Viewport&, const Allocation&, const MouseEvent& );
		Result handleMouseDrag( Viewport&, const Allocation&, const DragEvent& );
	private:
		void damage( Viewport& );
		Twips d_left, d_top, d_right, d_bottom;
		QColor d_color;
	};
}
