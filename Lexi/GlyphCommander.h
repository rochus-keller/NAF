/*
 * Copyright 2000-2015 Rochus Keller <mailto:rkeller@nmr.ch>
 *
 * This file is part of the CARA (Computer Aided Resonance Assignment,
 * see <http://cara.nmr.ch/>) NMR Application Framework (NAF) library.
 *
 * The following is the license that applies to this copy of the
 * library. For a license to use the library under conditions
 * other than those described here, please email to rkeller@nmr.ch.
 *
 * GNU General Public License Usage
 * This file may be used under the terms of the GNU General Public
 * License (GPL) versions 2.0 or 3.0 as published by the Free Software
 * Foundation and appearing in the file LICENSE.GPL included in
 * the packaging of this file. Please review the following information
 * to ensure GNU General Public Licensing requirements will be met:
 * http://www.fsf.org/licensing/licenses/info/GPLv2.html and
 * http://www.gnu.org/copyleft/gpl.html.
 */

#if !defined(AFX_GLYPHCOMMANDER_H__AF3E0D0A_7ADE_4D56_84DE_97B57856FAA1__INCLUDED_)
#define AFX_GLYPHCOMMANDER_H__AF3E0D0A_7ADE_4D56_84DE_97B57856FAA1__INCLUDED_

#if _MSC_VER >= 1000
#pragma once
#endif // _MSC_VER >= 1000

#include <Lexi/Handler.h>
#include <Lexi/GlyphCommand.h>

namespace Lexi
{
	class GlyphCommander : public Handler
	{
	public:
		GlyphCommander(Glyph* body = nil);
		virtual ~GlyphCommander();

		bool installCommand( GlyphCommand*, bool forward = true );
		//. Diese Methode wird in den abgeleiteten Klassen im Sinne einer Factory aufgerufen.
		//. Dem neuen Command wird d_alloc und d_trav präsentiert falls forward
		void registerForUndo( Command* ) const;
		//. Diese Methode sendet das übergebene Command an den Undo-Manager, falls es
		//. undoable ist.

		//* Overrides von Glyph
		void draw( Canvas&, const Allocation&);
		bool allocation( Allocation&, GlyphIndex = NilGlyphIndex ) const;
	private:
		Root::Ref<GlyphCommand> d_command;
		Allocation d_alloc;
		GlyphTraversal* d_trav;
	protected:
		void resetLastEvent();
		bool feedCommand();
		GlyphCommand* getCommand() const { return d_command; }
		void setLastEvent( const Allocation & a, GlyphTraversal & t );

		//* Overrides von Glyph, soll mit Bedacht überschrieben werden
		bool handle( const Allocation&, GlyphTraversal&);
		void handle( Message & msg ) { Handler::handle( msg ); }
	};
}

#endif // !defined(AFX_GLYPHCOMMANDER_H__AF3E0D0A_7ADE_4D56_84DE_97B57856FAA1__INCLUDED_)
